...
def arguments():
    ...
    a = argparse.ArgumentParser(description="Scan Tool")
    a.add_argument("-d", "--domain", help="Domain to start", required=False)
...
if __name__ == "__main__":
    ...
    if parsing.output:
        ...
        if os.path.isfile(reportPath):
            ...
            with open(reportPath, "w") as f:
                f.write(f"# TOOL REPORT FROM {domain.upper()}\n\n")
                f.close()
    ...
    if parsing.all:
        ...
        try:
            ...
            sys.exit(f"[{YELLOW}!{RESET}] Interrupt detected, exiting...\n")
        sys.exit(0)
